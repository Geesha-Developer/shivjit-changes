                                ...
                                <tbody>
                                    ...
                                    @foreach($customers as $customer)
                                        <tr>
                                            ...
                                            <td class="dynamic-data">{{ $customer->created_at->format('Y-m-d') }}</td>
                                            ...
                                            <td class="dynamic-data">
                                                @if($customer->status == 'Approved')
                                                    ${{ max(0, $customer->remaining_credit) }}
                                                @else
                                                    xxx
                                                @endif
                                            </td>
                                            
                                            <td class="dynamic-data">{{ $customer->customer_status }}</td>
                                            <td class="dynamic-data">
                                            @if(!empty($customer->commenter_name) && !empty($customer->comment_notes))
                                                @php
                                                    $commenterNames = json_decode($customer->commenter_name, true); // Decode as an associative array
                                                    $commentNotes = json_decode($customer->comment_notes, true);
                                                    $comments = '';

                                                    if (is_array($commenterNames) && is_array($commentNotes) && count($commenterNames) === count($commentNotes)) {
